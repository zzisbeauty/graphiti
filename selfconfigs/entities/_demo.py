...
class Concept(BaseModel):  
    ...
    代表独立的名词或名词短语,是知识图谱中可被多次引用的节点。  
      
    提取指南:  
    - 识别文本中明确提到的术语、概念名称、主题  
    - 通常是名词或名词短语,不是完整的句子  
    - 每个 Concept 应该是独立的、可被多次引用的知识点  
    - 适用于任何领域:学术、技术、商业、社会、艺术、生活等  
    - 例如:"CPU"、"计算机存储技术"、"企业创新机制"、"人际信任"  
    """  
...
class Insight(BaseModel):  
    ...
    Insight 表示用户在理解、思考或表达过程中形成的一个独立主题认知单元。  
    它是对某个主题、现象、经验或知识片段的综合性表达,体现用户对信息的理解结构。  
      
    **特点**:  
    - 可以是知识总结、观察结论、经验反思、理论说明或观点陈述  
    - 是语义上完整、聚焦的认知单元  
    - 可以是单个句子,也可以是多个句子组成的段落  
    - 适用于任何领域:学术、技术、商业、社会、艺术、生活等  
      
    **提取指南**:  
    1. 根据语义主题或逻辑焦点划分,而非段落或句法结构  
       - 如"CPU结构"、"企业创新机制"、"人际信任的形成"都可形成独立 Insight  
    2. 每个 Insight 聚焦一个核心议题,包含 1-2 个核心 Concept  
    3. 长文本中出现多个不同主题或视角时,应拆分为多个 Insight  
    4. 如果一个句子表达了完整的独立知识点,可以单独提取为一个 Insight  
    5. 如果多个句子共同支撑一个中心主题,可以合并为一个 Insight  
    6. 不要求含有主观标记(如"我认为"),客观陈述亦可  
      
    **示例**:  
    - 单句 Insight:"CPU是计算机的核心部件之一" (涉及"CPU"这1个 Concept)  
    - 多句 Insight:"CPU的主要组成部分包括控制单元、算术逻辑单元和寄存器。控制单元负责从内存中获取指令..." (涉及"CPU"、"控制单元"、"算术逻辑单元"、"寄存器"等多个 Concept)  
      
    **抽取目标**:  
    - 识别语义上自洽、主题明确的认知单元  
    - 为后续的主题聚合、认知追踪或语义索引提供基础  
    """  
